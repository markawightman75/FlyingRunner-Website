 ...
 * @package Genesis\Widgets
 */
class Sub_Featured_Posts extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'Displays smaller featured posts in columns with thumbnails', 'custom' ),
		);

		$control_ops = array(
			'id_base' => 'sub-featured-posts',
			'width'   => 505,
			'height'  => 350,
		);
		...
		parent::__construct( 'sub-featured-posts', __( 'Custom - Sub Featured Posts', 'custom' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			genesis_markup( array(
				'html5'   => '<article %s>',
				'xhtml'   => sprintf( '<div class="%s">', $classes_string),
				'context' => 'entry',
			) );
			...
			if ( $instance['show_title'] )
				...
				if ( ! empty( $instance['show_title'] ) ) {
					...
					$short_title_clean = strip_tags($short_title);
					$short_title_clean = esc_attr($short_title_clean);
					if ( ! empty ($short_title))
					{
						if ( genesis_html5() )
							printf( '<h2 class="entry-title"><a href="%s" title="%s">%s</a></h2>', get_permalink(), $short_title_clean, $short_title );
						else
							printf( '<h2><a href="%s" title="%s">%s</a></h2>', get_permalink(), $short_title_clean, $short_title );						
					}
					else
					{
						if ( genesis_html5() )
