...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '3.0.0', __( 'WooCommerce Checkout Add-Ons', 'woocommerce-checkout-add-ons' ), __FILE__, 'init_woocommerce_checkout_add_ons', array( 'minimum_wc_version' => '2.1', 'backwards_compatible' => '3.0.0' ) );
...
class WC_Checkout_Add_Ons extends SV_WC_Plugin {
	...
	public function add_json_search_field() {
		...
		// the field to search
		$id = urldecode( stripslashes( strip_tags( $_GET['add_on_id'] ) ) );
	...
	public function add_order_item_values( $_, $item, $item_id ) {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_2() ) {
			...
			if ( isset( $item['wc_checkout_add_on_id'] ) && $add_on = $this->get_add_on( $item['wc_checkout_add_on_id'] ) ) {
	...
	public static function save_order_item_values_ajax() {
		...
		if ( isset( $_POST['order_id'] ) && isset( $_POST['items'] ) ) {
			...
			// Save order items
			$GLOBALS['wc_checkout_add_ons']->save_order_item_values( $order_id, $items );
		}
 ...
 */
$GLOBALS['wc_checkout_add_ons'] = new WC_Checkout_Add_Ons();
