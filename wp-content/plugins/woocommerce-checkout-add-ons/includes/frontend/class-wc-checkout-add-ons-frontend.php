...
class WC_Checkout_Add_Ons_Frontend {
	...
	public function add_checkout_fields( $checkout_fields ) {
		...
		$add_ons = $GLOBALS['wc_checkout_add_ons']->get_add_ons();
	...
	public function render_add_ons() {
		...
		$this->load_styles_scripts( $checkout_add_on_fields );
		...
		wc_get_template(
			...
			'',
			$GLOBALS['wc_checkout_add_ons']->get_plugin_path() . '/templates/'
		);
	}
	...
	public function display_add_on_value_in_checkout_order_review( $safe_text, $text ) {
		...
		if ( isset( WC()->session->checkout_add_ons['fees'][ sanitize_title( $text ) ] ) ) {
			...
			$add_on = $GLOBALS['wc_checkout_add_ons']->get_add_on( $session_data['id'] );
			...
			if ( 'fee' == $item_type ) {
				...
				if ( $add_on_id ) {
					...
					if ( ! $label ) {
						...
						$add_on = $GLOBALS['wc_checkout_add_ons']->get_add_on( $add_on_id );
	 ...
	 *
	 * @since 1.0
	 * @param bool $excl_free true is free item meta should be exclided
