...
class WC_Checkout_Add_Ons_Shop_Order_CPT {
	...
	public function render_column_titles( $columns ) {
		...
		// inject our columns
		foreach ( $GLOBALS['wc_checkout_add_ons']->get_add_ons() as $add_on ) {
	...
	public function render_column_content( $column ) {
		...
		foreach ( $GLOBALS['wc_checkout_add_ons']->get_add_ons( $post->ID ) as $add_on ) {
		...
		foreach ( $GLOBALS['wc_checkout_add_ons']->get_add_ons() as $add_on ) {
			...
			if ( $add_on->is_filterable() ) {
				...
				if ( $add_on->has_options() ) {
					...
					?>
					<select name="<?php echo esc_attr( $add_on->get_key() ); ?>" id="<?php echo esc_attr( $add_on->get_key() ); ?>">
						<option value=""><?php _e( "Show all " . $add_on->name, WC_Checkout_Add_Ons::TEXT_DOMAIN ); ?></option>
						<?php
					...
					<?php
					$javascript .= "jQuery('select#" . $add_on->get_key() . "').chosen();";

				} elseif ( $add_on->type == 'text' ) {

					// search box dropdown
					?>
					<select id="<?php echo esc_attr( $add_on->get_key() ); ?>" name="<?php echo esc_attr( $add_on->get_key() ); ?>">
						<option value=""><?php _e( "Show all " . $add_on->label, WC_Checkout_Add_Ons::TEXT_DOMAIN ); ?></option>
						<?php
					...
					$javascript .= "
					jQuery( 'select#" . $add_on->get_key() . "' ).ajaxChosen( {
						method:         'GET',
						...
						data: {
							...
							default:    '" . addslashes( __( 'Show all ' . $add_on->name, WC_Checkout_Add_Ons::TEXT_DOMAIN ) ) . "'
						}
					}, function ( data ) {
		...
		foreach ( $GLOBALS['wc_checkout_add_ons']->get_add_ons() as $add_on ) {
			...
			if ( $add_on->is_filterable() && isset( $_GET[ $add_on->get_key() ] ) && $_GET[ $add_on->get_key() ] ) {
				...
				switch ( $add_on->type ) {
					...
					case 'multiselect':
					case 'multicheckbox':
						$where .= $wpdb->prepare( " AND woim_value.meta_value LIKE %s ", '%' . like_escape( $value ) . '%' );
					break;
	...
	public function build_formatted_names( $items, $order ) {
		...
		foreach ( $items as $key => $item ) {
			if ( 'fee' == $item['type'] && $item['wc_checkout_add_on_id'] ) {
				...
				$add_on = $GLOBALS['wc_checkout_add_ons']->get_add_on( $item['wc_checkout_add_on_id'] );
